...
| Interactive Dashboard  | Partial        | `internal/ui` is the real entrypoint now: a splash screen (block-letter DEVFLOW banner) hands off to a keyboard-navigable project list (favorite marker, tech stack, path, Docker badge). Still pending: a project detail view (Git status, commands, Docker containers/images), command output streaming, and a tmux session view. |
