...
// buildMascotArt assembles one full 8-row mascot bitmap from its three
// mascot-specific rows, filling in the shared headRow/gapRow rows
// around them.
//
// Parameters:
...
//     mascot this is.
//   - eyes: the row used twice for the eye band.
...
// mascotWidth.
func buildMascotArt(topper, eyes, foot string) []string {
...
// its accent color, and its static block-art bitmap.
type mascotSpec struct {
	...
	// Art holds the mascot's bitmap (see buildMascotArt) — a single
	// fixed pose, deliberately not animated, so the splash stays cheap
	// to render (no tea.Tick, no per-frame state).
	Art []string
}
...
var mascots = [4]mascotSpec{
	{ // git, top-left: a single center antenna (a branching motif),
		// round paired eyes, one trunk-like foot.
		Name:  "GitMaster",
	...
	{ // docker, top-right: no topper at all (a flush, flat roof, like
		...
		Art:   buildMascotArt(gapRow, "##.........##", headRow),
	},
	{ // tmux, bottom-left: twin toppers (representing split panes),
		// four small grid-arranged eyes, and twin feet.
		Name:  "PaneMan",
		Color: lipgloss.AdaptiveColor{Light: "#15803D", Dark: "#4ADE80"},
		Art:   buildMascotArt("...#.....#...", ".##.##.##.##.", ".....#.#....."),
	},
	{ // projects, bottom-right: an asymmetric folder-tab topper, a
		// single wide "cyclops" eye, and twin feet.
		Name:  "ProjectPal",
...
// renderMascotArt expands a mascot's bitmap (a mascotSpec's Art field)
// into block-art text, using the same dot-matrix technique
// banner.Render uses for the DEVFLOW wordmark: each bitmap cell becomes
// a filled ("██") or empty ("  ") pair of terminal columns (two columns
// per cell keeps the shape roughly square, since terminal cells are
// taller than they are wide).
//
// Parameters:
//   - rows: a mascotSpec's Art — mascotHeight rows of mascotBitmapWidth
//     cells each.
...
// renderCorner renders one full corner block for spec: its art, a
// blank line, and its name caption underneath — both in spec.Color.
// This is what View places in each of the four screen corners.
func renderCorner(spec mascotSpec) string {
	style := lipgloss.NewStyle().Foreground(spec.Color)
	art := renderMascotArt(spec.Art, style)
...
// four mascots (see the mascots slice) — its static art plus its name
// caption — in each screen corner in mascots' order (top-left,
// top-right, bottom-left, bottom-right), so the splash reads as a small
...
func (m Model) View() string {
	...
	topLeft := renderCorner(mascots[0])
	topRight := renderCorner(mascots[1])
	bottomLeft := renderCorner(mascots[2])
	bottomRight := renderCorner(mascots[3])
